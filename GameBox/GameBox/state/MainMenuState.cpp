...
MainMenuState::MainMenuState(Game* pGame) : State(States::MainMenu, pGame) {
	...
	m_buttons.push_back(Button("Start Game", pGame->GetFont(), [&]() {m_game->SetState(States::Game); }));
	m_buttons.back().setPosition(posX, posY);
	posY += 50;
	m_buttons.push_back(Button("Settings (not implemented)", pGame->GetFont(), [&]() {}));
	m_buttons.back().setPosition(posX, posY);
	posY += 50;
	m_buttons.push_back(Button("About (not implemented)", pGame->GetFont(), [&]() {}));
	m_buttons.back().setPosition(posX, posY);
	posY += 50;
	m_buttons.push_back(Button("Quit", pGame->GetFont(), [&]() {exit(0); }));
	m_buttons.back().setPosition(posX, posY);
	posY += 50;
